...
acc =   numeric(N)
j=1
for(i in seq(from=0,to=1,length.out=N)){
  
  pred_tf_cut = preds_lasso>i
  acc_frame_cut = tibble(pred=pred_tf_cut,true=Y_test)
  conf_mat_cut = (acc_frame_cut %>% group_by(true,pred) %>% count())$n %>% matrix(ncol = 2,nrow=2)
  precision_cut= c(conf_mat_cut[1,1]/sum(conf_mat_cut[,1]),conf_mat_cut[2,2]/sum(conf_mat_cut[,2]))
  recall_cut = c(conf_mat_cut[1,1]/sum(conf_mat_cut[1,]),conf_mat_cut[2,2]/sum(conf_mat_cut[2,]))
  acc_mat_cut = pred_tf_cut == Y_test
  acc_cut = sum(acc_mat_cut)/length(acc_mat_cut) 
  cutoff[j] = i
  prec1[j] =precision_cut[1]
  prec2[j] =precision_cut[2]
  rec1[j] =recall_cut[1]
  rec2[j] =recall_cut[2]
  acc[j] = acc_cut
  j = 1+j
}
cut_frame = tibble(cutoff=cutoff,prec1=prec1,prec2=prec2,rec1=rec1,rec2=rec2,acc=acc)
ggplot(cut_frame,aes(x=cutoff)) + geom_point(aes(y=prec1,colour='red')) + geom_point(aes(y=prec2,colour='blue')) + geom_point(aes(y=rec1,colour='yellow')) + geom_point(aes(y=rec2,colour='green')) + geom_point(aes(y=acc,colour='black'))
