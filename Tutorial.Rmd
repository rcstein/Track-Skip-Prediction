...
```{r}
confusion.glmnet(lasso,newx=X_lasso_train,newy = Y_lasso_train)
measure_lasso = assess.glmnet(lasso,newx=X_lasso_train,newy = Y_lasso_train)
measure_lasso$auc
```
Unfortunately, It does not seem to have changed our metrics much, but it at least informs us about what data seems unnecessary.
...
```{r}
temp = subset(total_data,select=-c(not_skipped,skip_1,skip_2,skip_3,session_id,track_id_clean,date))

cl = sapply(temp, class)
ps = sapply(cl, function(x){
  return(ifelse(x=='factor','-1',''))
})
f_tot =  paste(colnames(temp),ps,collapse = " + ",sep = '')
form_tot = as.formula(paste(c('~ ',f_tot),collapse = '',sep=''))
#>>>>>>>>>>>>>
X_tot = as.data.frame(model.matrix(form_tot,data=total_data))
```
With that done we can now make predictions with glmnet!
```{r}
X_tot_test = X_tot[indx,]
X_tot_train = X_tot[-indx,]
Y_tot_test = total_data$skip_1[indx]
Y_tot_train = total_data$skip_1[-indx]

#lasso = cv.glmnet(as.matrix(X_tot_train),Y_tot_train,family = 'binomial',alpha = 1)
```
