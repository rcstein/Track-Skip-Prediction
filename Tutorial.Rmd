...
?build_lcm()
```
State 4 seems to indicate non-skipping behavior while state 3 is associated with pressing the forward button. Based on the distribution, this may actually be a Markov Model, which is a special case of the HMM where the hidden states correspond perfectly with the observed states (i.e. there are no hidden states.)
