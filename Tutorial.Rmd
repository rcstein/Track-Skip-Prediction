...
conf_mat = (frame %>% group_by(true,pred) %>% count())$freq %>% matrix(ncol = 2,nrow=2,dimnames = list(c('True','False'),c('True','False')))
conf_mat
```
...
conf_mat_numeric = (frame %>% group_by(true,pred) %>% count())$freq%>% matrix(ncol = 2,nrow=2,dimnames = list(c('True','False'),c('True','False')))
conf_mat_numeric
...
```{r}
library(dplyr)
as.data.frame(total_data) %>% dplyr::group_by(skip_1,skip_2,skip_3,not_skipped) %>% dplyr::count()
```
...
dim(total_data)
total_data2 = total_data %>% dplyr::filter(!(skip_2==TRUE & total_data$skip_3==FALSE))
dim(total_data2)
total_data2 %>% dplyr::group_by(skip_1,skip_2,skip_3,not_skipped) %>% dplyr::count()
...
total_data2 %>% dplyr::group_by(skip_1,skip_2,skip_3,not_skipped,listen_time) %>% dplyr::count()
```
With this accomplished we need consider what tools we can use to model this. There are many ways to go about a multiple classification problem. One is to do one-vs.-rest modeling where you fit a logistic function to each category and then classify that observation as whichever class had the highest predicted probability. This is also known as Multinomial Logistic Regression, and once again glmnet has a function for this. All you have to do is change the family parameter on your glmnet model from 'binomial' to 'multinomial'. (Note: Because AUC relies on having True positives and False positives, it no longer makes sense in a multi-class setting and can no longer be applied)
...
```
```{r,eval=FALSE}
Full_precision2 = listen_conf3[1,1]/sum(listen_conf3[,1])

long_precision2 = listen_conf3[2,2]/sum(listen_conf3[,2])

med_precision2 = listen_conf3[3,3]/sum(listen_conf3[,3])

short_precision2 = listen_conf3[4,4]/sum(listen_conf3[,4])

print(list(full=Full_precision2,long= long_precision2,med = med_precision2, short= short_precision2))
```
```{r,include=FALSE}
# The model takes a very long time to run so a screenshot of the results was taken to allow for faster knitting
knitr::include_graphics('prec2.png')
...
```{r define sequences,echo=FALSE,message=FALSE}
...
Below are plots of a selection of state channels. 
```{r plots, fig.height = 14, fig.width=8, message=FALSE}
# Plots
...
skip_fit$model
ssplot(skip_fit$model, plots = "hidden.paths", type = "I")
```


Mixture Markov models can be used to cluster entire sequences, rather than individual states. **seqHMM** implements clustering as a special case of MHMMs where the only hidden state is class (*see*: Helske et al.). We fit such a model below using multiple channels and covariates, which can be incorporated into prior probabilities during computation. 

## Time Series modeling

Now that we've analyzed this data as a sequence we know that there are some features that definitely seem to affect each other over time. For simplicity I've decided to only add in one of these: listen_time and to only use lag-1. Additionally for the first states in a sequence I will assume that they have the same lag_listen_time state as their listen_time

```{r time series}
dim(total_data2)
total_data2 %>% dplyr::group_by(session_position) %>% dplyr::count()
lag1 = tibble(session_id = total_data2$session_id,
              session_position = total_data2$session_position,
              lag_listen_time = total_data2$listen_time)
dim(lag1)
sum((lag1 %>% dplyr::group_by(session_id,session_position) %>% dplyr::count())$n > 1)
lag1$session_position = lag1$session_position+1
sum((lag1 %>% dplyr::group_by(session_id,session_position) %>% dplyr::count())$n > 1)
dim(lag1)

total_data4 = left_join(total_data2,lag1,by=c('session_id','session_position'))
dim(total_data4)
 
for(i in 1:dim(total_data4)[1]){
  if(is.na(total_data4$lag_listen_time[i])){
    total_data4$lag_listen_time[i] = total_data4$listen_time[i]
  }
}
##to prove this worked
sum((total_data4 %>% dplyr::group_by(session_position) %>% dplyr::count())$n == (total_data4 %>% dplyr::group_by(session_position) %>% dplyr::count())$n)
total_data4$lag_listen_time = as.factor(total_data4$lag_listen_time)
```
```{r}
temp_data_listen4 = subset(total_data4,select=-c(not_skipped,skip_1,skip_2,skip_3,session_id,track_id_clean,date,listen_time))

cl_listen4 = sapply(temp_data_listen4, class)
ps_listen4 = sapply(cl_listen4, function(x){
  return(ifelse(x=='factor','-1',''))
})
f_listen4 =  paste(colnames(temp_data_listen4),ps_listen4,collapse = " + ",sep = '')
fla_listen4 = as.formula(paste(c('~ ',f_listen4),collapse = '',sep=''))
#>>>>>>>>>>>>>
X_listen4 = model.matrix(fla_listen4,data=total_data4)

indx_listen4 = sample(length(total_data4$listen_time),length(total_data4$listen_time)%/%10)
X_listen_train4 = X_listen4[-indx_listen4,]
X_listen_test4 = X_listen4[indx_listen4,]
Y_listen_train4 = total_data4$listen_time[-indx_listen4]
Y_listen_test4 = total_data4$listen_time[indx_listen4]
```
```{r,eval=FALSE}
time_series = cv.glmnet(X_listen_train4,Y_listen_train4,family='multinomial',alpha=1)
```
```{r,eval=FALSE}
time_series_conf = confusion.glmnet(time_series,newx = X_listen_test4,newy=Y_listen_test4)
time_series_conf
```
```{r,include=FALSE}
# The model takes a very long time to run so a screenshot of the results was taken to allow for faster knitting
knitr::include_graphics('conf3.png')

```
```{r,eval=FALSE}
Full_precision = time_series_conf[1,1]/sum(time_series_conf[,1])

long_precision = time_series_conf[2,2]/sum(time_series_conf[,2])

med_precision = time_series_conf[3,3]/sum(time_series_conf[,3])

short_precision = time_series_conf[4,4]/sum(time_series_conf[,4])

print(list(full=Full_precision,long= long_precision,med = med_precision, short= short_precision))
```
```{r,include=FALSE}
# The model takes a very long time to run so a screenshot of the results was taken to allow for faster knitting
knitr::include_graphics('prec3.png')

```
You can see here that the precision for both med and long increase as well as the overall accuracy. The short precision dropped slightly and the Full precision was more or less unchanged. You can imagine that by adding in more sequence terms for listen_time as well as for other categoires we could further increase our prediction accuracy.
# References
