...
conf_mat = (frame %>% dplyr::group_by(true,pred) %>% dplyr::count())$n %>% matrix(ncol = 2,nrow=2,dimnames = list(c('True','False'),c('True','False')))
conf_mat
```
...
conf_mat_numeric = (frame %>% dplyr::group_by(true,pred) %>% dplyr::count())$n%>% matrix(ncol = 2,nrow=2,dimnames = list(c('True','False'),c('True','False')))
conf_mat_numeric
...
```{r, fig.height = 14, fig.width=8, echo = TRUE}
# An example of defining one ssp (plot) object
plot_ub1 <- ssp(list(skip_sequence[200:400,], start_sequence[200:400,]), #setting number of sequences to plot
sortv = "from.start", sort.channel = 1, type = "I", # 'I' corresponds to observations, as opposed to a distribution. 
...
ncol.legend = c(1, 2)) # aesthetic properties
```
...
This code lays plot objects out on a grid. 
...
*Note: the fit_model function has been commented out as it can take quite awhile to run. Relavent output was saved to disk, but you can see the code that was used.
```{r hidden states}
...
Now that we've analyzed this data as a sequence and as an HMM we know that there are some features that definitely seem to affect each other over time. For simplicity I've decided to only add in one term as proof of concept (listen_time at lag-1). Additionally for the first states in a sequence I will assume that they have the same lag_listen_time state as their listen_time
