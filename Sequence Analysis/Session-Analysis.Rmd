...
```{r, eval = FALSE}
skip_model <- build_hmm(observations = channels, n_states = 4)
...
```{r}
channels <- list(skip_sequence, context_sequence, decade_sequence, dance_sequence, pop_sequence,speech_sequence)
session_clust <- build_lcm(channels, n_clusters = 4, formula = ~time + day, data = dance, channel_names = c("skips","context", "decade","dance", "pop", "speech"))
