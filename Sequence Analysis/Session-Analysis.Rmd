...
```{r}
channels <- list(skip_sequence, context_sequence, decade_sequence)
session_clust <- build_lcm(channels, n_clusters = 8, formula = ~time + day, data = dance, channel_names = c("skips","context", "decade"))
session_fit <- fit_model(session_clust,  em_step = FALSE, global_step = TRUE) 
session_fit$model

ssplot(session_fit$model)

plot(session_fit$model)
